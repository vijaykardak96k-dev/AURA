...
const stateDescriptions = {
  STARTING: 'Initializing AURA systems', ACTIVE: 'Listening for your command', HEARING: 'Hearing you…', SLEEPING: 'Say AURA to wake me',
  THINKING: 'Processing your request', SPEAKING: 'AURA is responding', EXECUTING: 'Executing your request',
  WAITING_FOR_CONFIRMATION: 'Waiting for your confirmation', WAITING_FOR_BROWSER_CHOICE: 'Which browser should I use?',
  ERROR: 'System requires attention', STOPPED: 'Voice listening is paused'
};
...
// AURA reports honestly whether it's running on Groq or on the offline
// parser — the header label must never claim a provider that isn't working.
window.aura?.onProviderStatus(data => {
  if (!data) return;
  const label = data.online ? (data.label || 'GROQ') : 'OFFLINE';
  updateAIEngine(label);
  document.body.dataset.provider = data.online ? 'online' : 'offline';
  if (data.detail) addLog(data.detail, data.online ? 'INFO' : 'WARN');
});

window.aura?.onSttStatus(data => {
  const ok = data?.available !== false;
  setConnectionDot('MICROPHONE', ok);
  addLog(ok ? 'Speech recognition ready.' : `Speech recognition unavailable: ${data?.error || 'unknown error'}`, ok ? 'INFO' : 'WARN');
});

window.aura?.onTtsStatus(data => {
  const ok = data?.available !== false;
  setConnectionDot('TTS', ok);
  addLog(ok ? 'Voice output ready.' : `Voice output unavailable: ${data?.error || 'unknown error'}`, ok ? 'INFO' : 'WARN');
});

// Task 2/8: per-utterance emotion + completion status. Reuses the
// existing dataset-attribute + log pattern above rather than adding new
// UI chrome — document.body.dataset.emotion lets the existing stylesheet
// opt into styling this later without another IPC round-trip.
window.aura?.onTtsUtterance(data => {
  if (!data || !data.state) return;
  document.body.dataset.emotion = data.emotion || 'neutral';
  if (data.state === 'speaking') {
    addLog(`Speaking (${data.emotion || 'neutral'})...`, 'INFO');
  } else if (data.state === 'failed') {
    addLog(`TTS failed after ${data.seconds ?? '?'}s.`, 'WARN');
  }
});

// Typing "mute" is the documented way to interrupt AURA mid-sentence.
// Route it straight to the backend's mute command so it doesn't have to
// queue behind whatever command is currently being processed.
function isMuteCommand(text) {
  return ['mute', 'stop talking', 'be quiet', 'silence'].includes(
...
function setConnectionDot(label, ok) {
  document.querySelectorAll('.connection').forEach(el => {
    if (!el.textContent.trim().toUpperCase().startsWith(label)) return;
