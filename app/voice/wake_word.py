...
fix for the "AURA falls through to the AI provider" bug: wake-word detection must
...
classification to the AI prompt (asking the model to "recognize wake
words") made unreliable in the first place. A language model asked to interpret "Aura" naturally
answers it conversationally instead of treating it as a control signal
— that's not a bug in the model, it's a category error in *where* wake-word
...
reaches parse_intent()/Groq. There is no code path from
