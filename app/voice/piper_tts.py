...
class PiperTextToSpeech:
    ...
    def speak(self, text: str) -> None:
        """
        Synthesizes `text` with the already-loaded voice and plays it —
        no subprocess, no temp file. If the voice hasn't been preloaded
        yet, this loads it now (first call pays the load cost instead of
        startup — still far better than paying it on EVERY call, which
        is what a subprocess-per-utterance integration does). Never
        raises — logs and returns quietly on any failure, since voice
        output is a convenience, not something other actions depend on.
        """
        ...
        try:
            ...
            logger.warning(f"Playback library missing for Piper output: {e}")
            return
        except OSError as e:
            logger.warning(f"Audio output unavailable for Piper (PortAudio): {e}")
            return

        try:
            from piper import SynthesisConfig
            syn_config = SynthesisConfig(volume=self.volume, length_scale=self.length_scale)
        except Exception:
            syn_config = None  # older/newer piper-tts versions — fall back to voice defaults
