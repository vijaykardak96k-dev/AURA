...
class TextToSpeech:
    ...
    def speak(self, text: str, emotion: Optional[str] = None, **kwargs) -> bool:
        ...
        `emotion` is accepted for interface parity with PiperTextToSpeech
        but not applied here — pyttsx3/SAPI5 has no comparable prosody
        knobs, so this is intentionally a no-op, not a missing feature.
        Any other keyword is accepted and ignored the same way."""
...
class ResilientTextToSpeech:
    """Primary Piper TTS with automatic pyttsx3 fallback on playback failure."""

    def __init__(self, primary):
        self.primary = primary
        self.fallback = TextToSpeech()

    def preload(self) -> None:
        try:
            self.primary.preload()
        except Exception as e:
            logger.warning("Primary TTS preload failed: %s", e)

    def speak(self, text: str, emotion: Optional[str] = None, **kwargs) -> bool:
        try:
            ok = self.primary.speak(text, emotion=emotion, **kwargs)
            if ok is not False:
                return True
        except TypeError:
            # Primary engine's speak() predates the emotion/**kwargs
            # signature (e.g. a custom/mocked engine in tests) — retry
            # with the plain call rather than failing the utterance.
            try:
                ok = self.primary.speak(text)
                if ok is not False:
                    return True
            except Exception as e:
                logger.warning("Primary TTS failed: %s", e)
        except Exception as e:
            logger.warning("Primary TTS failed: %s", e)

        logger.info("Falling back to Windows SAPI5 TTS.")
        return bool(self.fallback.speak(text, emotion=emotion))
...
def create_tts_engine(preferred: Optional[str] = None):
    ...
    if piper_model:
        ...
        try:
            piper.preload()
            return ResilientTextToSpeech(piper)
