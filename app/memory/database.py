...
def get_all_memory(db_path: Path = DB_PATH) -> list[dict]:
    with _connect(db_path) as conn:
        conn.row_factory = sqlite3.Row
        rows = conn.execute(
...
def get_actions_between(start_iso: str, end_iso: str | None = None, db_path: Path = DB_PATH) -> list[dict]:
    """Action-log rows with timestamp >= start_iso (and < end_iso if given),
    oldest first. Used to build "what did you do today/yesterday" summaries.
    """
    with _connect(db_path) as conn:
        conn.row_factory = sqlite3.Row
        if end_iso:
            rows = conn.execute(
                "SELECT * FROM action_log WHERE timestamp >= ? AND timestamp < ? ORDER BY id ASC",
                (start_iso, end_iso),
            ).fetchall()
        else:
            rows = conn.execute(
                "SELECT * FROM action_log WHERE timestamp >= ? ORDER BY id ASC",
                (start_iso,),
            ).fetchall()
        return [dict(r) for r in rows]
