...
whenever no AI provider is available (Groq unreachable/no key/rate
limited, Ollama not running, or AI_PROVIDER=fallback), so AURA never
...
def parse_fallback_to_dict(text: str) -> dict:
    ...
    questions are Groq's job (see prompt_builder.py's CONVERSE
