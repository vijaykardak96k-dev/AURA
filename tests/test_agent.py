...
def test_start_enters_sleeping_when_voice_enabled():
    """With a microphone, AURA starts SLEEPING and requires the wake word."""
...
def test_bare_wake_word_wakes_without_calling_the_provider():
    agent, provider, tts, stt = make_agent(voice_enabled=True, confirmation_timeout=2)
    agent.start()
    assert agent.state == AuraState.SLEEPING
...
def test_manual_start_listening_resumes_to_sleeping_requiring_wake_word():
    """Resuming after a manual stop goes back to SLEEPING — a deliberate
    safety default, not straight back to ACTIVE."""
    agent, provider, tts, stt = make_agent(
        scripted_response={"actions": [{"action": "CPU_INFO"}]},
        voice_enabled=True,
    )
