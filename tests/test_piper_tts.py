...
def test_speak_preloads_automatically_if_not_already(tmp_path, monkeypatch):
    ...
    tts = PiperTextToSpeech(model_path=str(model_file))
    # Deliberately no punctuation here: app/voice/prosody.py's
    # chunk_for_speech() only splits (and inserts pauses) at punctuation,
    # so a single clause stays one Piper synthesize() call — see
    # test_speak_inserts_pauses_between_punctuation_chunks below for the
    # multi-chunk/pause case.
...
def test_speak_inserts_pauses_between_punctuation_chunks(tmp_path, monkeypatch):
    """Task 2: 'Hello, welcome.' is two clauses -> two synthesize() calls
    -> two chunks of fake audio with a silence gap stitched between them,
    proving the prosody chunker is actually wired into speak()."""
    _patch_piper_module(monkeypatch)
    model_file = tmp_path / "voice.onnx"
    model_file.write_text("fake")
    ...
    tts = PiperTextToSpeech(model_path=str(model_file))
    tts.speak("Hello, welcome.", emotion="neutral")

    # Two clauses -> two synthesize() calls -> two 5-sample chunks (10
    # total) plus at least one silence gap in between, so strictly more
    # than the no-punctuation case's 5 samples and more than 10 (the
    # comma's pause is real, non-zero silence).
    assert len(played["audio"]) > 10


def test_speak_accepts_emotion_and_unknown_kwargs_without_raising(tmp_path, monkeypatch):
    """Backward compatibility (Task 1 requirement 4): tts.speak(text) and
    tts.speak(text, emotion=...) must both work, and an unrecognized
    kwarg must be ignored rather than raising TypeError."""
    _patch_piper_module(monkeypatch)
    model_file = tmp_path / "voice.onnx"
    model_file.write_text("fake")
    ...
    tts = PiperTextToSpeech(model_path=str(model_file))
    assert tts.speak("Plain call") is True
    assert tts.speak("With emotion", emotion="playful") is True
    assert tts.speak("With junk kwarg", emotion="calm", some_future_option=True) is True
